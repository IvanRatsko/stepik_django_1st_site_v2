...
class MainView(View):
    def get(self, request, *args, **kwargs):
        context = {}
        random_six_tours = random.sample([i for i in range(1, len(tours))], 6)
        context["tours"] = {random_i: tours[random_i] for random_i in random_six_tours}
    ...
    def get(self, request, departure, *args, **kwargs):
        ...
        for key, value in tours.items():
            if value["departure"] == departure:
                tours_filtred[key] = value
                prices.append(value["price"])
                nights.append(value["nights"])
        context["tours"] = tours_filtred
        context["prices"] = {"min": min(prices), "max": max(prices)}
        context["nights"] = {"min": min(nights), "max": max(nights)}
        context["tours_count"] = len(tours_filtred)
        context["departures"] = departures
